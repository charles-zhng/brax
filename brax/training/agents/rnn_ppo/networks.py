...
def make_inference_fn(rnn_ppo_networks: RNNPPONetworks, compute_value: bool = False):
  ...
  def make_policy(
    ...
    Returns a function that takes (observations, hidden_state, key) and returns
    ...
    def policy(
        ...
        key_sample: PRNGKey,
    ) -> Tuple[types.Action, types.Extra, Tuple[HiddenState, HiddenState]]:
      ...
      Returns:
        Tuple of (actions, extras, new_hidden_states)
      """
      policy_hidden, value_hidden = hidden_states
      ...
      if deterministic:
        ...
        new_value_hidden = value_hidden
        extras = {'policy_hidden': new_policy_hidden, 'value_hidden': new_value_hidden}
        return actions, extras, (new_policy_hidden, new_value_hidden)
      ...
      if compute_value:
        value, new_value_hidden = rnn_ppo_networks.value_network.apply(
            params[0], params[2], observations, value_hidden
        )
        extras['value'] = value
        extras['value_hidden'] = new_value_hidden
      else:
        new_value_hidden = value_hidden

      return postprocessed_actions, extras, (new_policy_hidden, new_value_hidden)
