...
def _reset_hidden_on_done(
  ...
  Args:
    ...
    rnn_ppo_network: The RNN-PPO networks (for getting hidden size)
    is_policy: Whether this is for policy network (True) or value network
...
def actor_step_rnn(
...
) -> Tuple[envs.State, types.Transition, Tuple[HiddenState, HiddenState]]:
  ...
  Args:
    ...
    hidden_states: Tuple of (policy_hidden, value_hidden)
    key: Random key
    extra_fields: Extra fields to collect from env state
    rnn_ppo_network: RNN-PPO networks for hidden state reset

  Returns:
    Tuple of (next_state, transition, new_hidden_states)
  """
  actions, policy_extras, new_hidden_states = policy(
      env_state.obs, hidden_states, key
  )
  nstate = env.step(env_state, actions)
  state_extras = {x: nstate.info[x] for x in extra_fields}

  # Store the initial hidden states for this step (before the step was taken)
  policy_extras['initial_policy_hidden'] = hidden_states[0]
  policy_extras['initial_value_hidden'] = hidden_states[1]
  ...
  if rnn_ppo_network is not None:
    policy_hidden, value_hidden = new_hidden_states
    policy_hidden = _reset_hidden_on_done(
        policy_hidden, nstate.done, rnn_ppo_network, is_policy=True
    )
    value_hidden = _reset_hidden_on_done(
        value_hidden, nstate.done, rnn_ppo_network, is_policy=False
    )
    new_hidden_states = (policy_hidden, value_hidden)

  return nstate, transition, new_hidden_states
...
def generate_unroll_rnn(
  ...
  Returns:
    Tuple of (final_state, transitions, final_hidden_states)
  """
  ...
  (final_state, final_hidden, _), data = jax.lax.scan(
      f, (env_state, hidden_states, key), (), length=unroll_length
  )
...
class RecurrentEvaluator:
  ...
  def __init__(
    ...
    def generate_eval_unroll(
      ...
      value_hidden = rnn_ppo_network.value_network.init_hidden(num_eval_envs)
...
def train(
  ...
  def training_step(
      carry: Tuple[TrainingState, envs.State, Tuple[HiddenState, HiddenState], PRNGKey],
      unused_t,
  ) -> Tuple[Tuple[TrainingState, envs.State, Tuple[HiddenState, HiddenState], PRNGKey], Metrics]:
    training_state, state, hidden_states, key = carry
    ...
    def f(carry, unused_t):
      ...
      return (next_state, new_hidden, next_key), data

    (state, hidden_states, _), data = jax.lax.scan(
        f,
        (state, hidden_states, key_generate_unroll),
        (),
    ...
    return (new_training_state, state, hidden_states, new_key), metrics
  ...
  def training_epoch(
      ...
      hidden_states: Tuple[HiddenState, HiddenState],
      key: PRNGKey,
  ) -> Tuple[TrainingState, envs.State, Tuple[HiddenState, HiddenState], Metrics]:
    (training_state, state, hidden_states, _), loss_metrics = jax.lax.scan(
        training_step,
        (training_state, state, hidden_states, key),
        (),
        length=num_training_steps_per_epoch,
    )
    loss_metrics = jax.tree_util.tree_map(jnp.mean, loss_metrics)
    return training_state, state, hidden_states, loss_metrics
  ...
  def training_epoch_with_timing(
    ...
    t = time.time()
    training_state, env_state, hidden_states = _strip_weak_type(
        (training_state, env_state, hidden_states)
    )
    result = training_epoch(training_state, env_state, hidden_states, key)
    training_state, env_state, hidden_states, metrics = _strip_weak_type(result)
    ...
    return training_state, env_state, hidden_states, metrics
  ...
  def init_hidden_for_training(init_fn):
    hidden = init_fn(envs_per_device)
    # Replicate across devices
    return jax.device_put_replicated(
        hidden, jax.local_devices()[:local_devices_to_use]
    )

  policy_hidden = init_hidden_for_training(
      rnn_ppo_network.policy_network.init_hidden
  )
  value_hidden = init_hidden_for_training(
      rnn_ppo_network.value_network.init_hidden
  )
  hidden_states = (policy_hidden, value_hidden)
  ...
  for it in range(num_evals_after_init):
    ...
    for _ in range(max(num_resets_per_eval, 1)):
      ...
      (training_state, env_state, hidden_states, training_metrics) = (
          training_epoch_with_timing(
              training_state, env_state, hidden_states, epoch_keys
          )
      )
      ...
      if num_resets_per_eval > 0:
        ...
        # Reset hidden states on env reset
        policy_hidden = init_hidden_for_training(
            rnn_ppo_network.policy_network.init_hidden
        )
        value_hidden = init_hidden_for_training(
