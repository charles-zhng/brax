...
class RNNPPOTest(parameterized.TestCase):
  ...
  def testResetHiddenOnDone(self, cell_type):
    ...
    # Reset hidden states
    new_hidden = rnn_ppo_train._reset_hidden_on_done(
        hidden, done, rnn_ppo_network, is_policy=True
    )
  ...
  def testResetHiddenOnDoneWithNone(self):
    """Test _reset_hidden_on_done handles None hidden state (value network)."""
    from brax.training.agents.recurrent_ppo import train as rnn_ppo_train
    ...
    result = rnn_ppo_train._reset_hidden_on_done(
        None, done, rnn_ppo_network, is_policy=False
    )
  ...
  def testScanForwardMatchesSequential(self):
    ...
    obs_seq = jax.random.normal(
        key, (seq_length, batch_size, env.observation_size)
    )
    ...
    # Compare outputs
    self.assertTrue(jnp.allclose(scan_output, step_output, atol=1e-5))
    if isinstance(hidden, tuple):
      for h1, h2 in zip(scan_final_hidden, hidden):
        self.assertTrue(jnp.allclose(h1, h2, atol=1e-5))
    else:
      self.assertTrue(jnp.allclose(scan_final_hidden, hidden, atol=1e-5))
  ...
  def testLossComputationSequenceIntegrity(self):
    ...
    next_obs = jax.random.normal(key, (batch_size, seq_length, env.observation_size))
