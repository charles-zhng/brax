        ...
        def generate_eval_unroll(
            ...
            def step_fn(carry, unused_t):
                ...
                truncation = next_state.info.get(
                    "truncation",
                    jnp.zeros_like(discount, dtype=discount.dtype),
                )
                mask = (1.0 - truncation) * discount
...
def _recurrent_actor_step(
    ...
    truncation = nstate.info.get(
        "truncation", jnp.zeros_like(discount, dtype=discount.dtype)
    )
    continuation_mask = (1.0 - truncation) * discount
    masked_core_state = _mask_core_state(new_core_state, continuation_mask, mask_fn)
...
def train(
    ...
    def training_step(
        ...
        def f(carry, unused_t):
            ...
            return (next_state, next_core_state, next_key), (
                data,
                core_history,
                next_core_state,
            )
    ...
    if restore_params is not None:
        ...
        value_params = (
            restore_params[1] if restore_value_fn else init_model_params.model
        )
        training_state = training_state.replace(
        ...
        for _ in range(max(num_resets_per_eval, 1)):
            ...
            (training_state, env_state, core_state, training_metrics) = (
                training_epoch_with_timing(
                    training_state, env_state, core_state, epoch_keys
                )
            )
