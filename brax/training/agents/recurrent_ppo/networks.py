    ...
    def __call__(
        ...
        if self.core_type == "gru":
            rnn_cell = linen.GRUCell(
                name="gru", features=self.hidden_size, kernel_init=self.kernel_init
            )
            ...
            lstm_cell = linen.LSTMCell(
                name="lstm", features=self.hidden_size, kernel_init=self.kernel_init
            )
            carry = (
...
def make_inference_fn(ppo_networks: RecurrentPPONetworks, compute_value: bool = True):
...
def make_recurrent_ppo_networks(
    ...
    obs_size = networks._get_obs_state_size(
        observation_size, policy_obs_key
